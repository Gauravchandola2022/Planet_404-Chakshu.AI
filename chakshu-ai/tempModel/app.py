...
def main():
    st.title("🪐 CHAKSHU.AI - Hunting for Exoplanets with AI")
    ...
    page = st.sidebar.selectbox("Choose a page", 
                               ["Train Model", "Make Predictions", "Model Performance", "Feature Analysis", "Light Curves"])
                ...
                if st.sidebar.button("Load Selected Version", type="primary"):
                    ...
                    if os.path.exists(version_dir):
                        try:
                            ...
                            st.sidebar.success("✅ Model version loaded!")
...
def generate_transit_lightcurve(period, duration, depth, num_points=1000, num_transits=3):
    """Generate a simulated transit light curve based on transit parameters"""
    total_time = period * num_transits
    time = np.linspace(0, total_time, num_points)
    flux = np.ones(num_points)
    
    for i in range(num_transits):
        transit_center = period * (i + 0.5)
        transit_half_duration = duration / 24 / 2
        
        in_transit = np.abs(time - transit_center) <= transit_half_duration
        
        if in_transit.any():
            for j in np.where(in_transit)[0]:
                phase = (time[j] - transit_center) / transit_half_duration
                if abs(phase) <= 1:
                    depth_fraction = depth / 1e6
                    transit_shape = np.sqrt(1 - phase**2)
                    flux[j] = 1 - (depth_fraction * transit_shape)
    
    noise = np.random.normal(0, depth/1e6/20, num_points)
    flux += noise
    
    return time, flux

def light_curves_page():
    st.header("🌟 Exoplanet Transit Light Curves")
    st.markdown("""
    Light curves show the brightness of a star over time. When an exoplanet passes in front of its host star 
    (a transit), it blocks a tiny fraction of the star's light, creating a characteristic dip in brightness.
    """)
    
    uploaded_file = st.file_uploader(
        "Upload NASA Exoplanet Dataset (CSV)", 
        type=['csv'],
        help="Upload the dataset to visualize light curves",
        key="lightcurve_uploader"
    )
    ...
    period_col = [col for col in df.columns if 'period' in col.lower() and 'err' not in col.lower()]
    duration_col = [col for col in df.columns if 'duration' in col.lower() and 'err' not in col.lower()]
    depth_col = [col for col in df.columns if 'depth' in col.lower() and 'err' not in col.lower()]
    disposition_col = [col for col in df.columns if 'disp' in col.lower() or 'status' in col.lower()]
    
    if not (period_col and duration_col and depth_col):
        st.error("Required columns (period, duration, depth) not found in dataset")
        return
    
    period_col = period_col[0]
    duration_col = duration_col[0]
    depth_col = depth_col[0]
    disposition_col = disposition_col[0] if disposition_col else None
    
    valid_data = df[[period_col, duration_col, depth_col]].notna().all(axis=1)
    df_valid = df[valid_data].copy()
    
    if len(df_valid) == 0:
        st.error("No valid data found with all required parameters")
        return
    
    st.success(f"Found {len(df_valid)} objects with complete transit parameters")
    ...
    with col1:
        if disposition_col:
            disposition_filter = st.multiselect(
                ...
                options=sorted(df_valid[disposition_col].unique()),
                default=sorted(df_valid[disposition_col].unique())[:3] if len(df_valid[disposition_col].unique()) >= 3 else sorted(df_valid[disposition_col].unique()),
                help="Select which object types to show"
            )
            df_filtered = df_valid[df_valid[disposition_col].isin(disposition_filter)]
        else:
            df_filtered = df_valid
    
    with col2:
        num_samples = st.slider("Number of Objects to Display", 1, min(20, len(df_filtered)), min(5, len(df_filtered)))
    
    if len(df_filtered) == 0:
        st.warning("No objects match the selected filters")
        return
    
    sample_df = df_filtered.sample(n=min(num_samples, len(df_filtered)), random_state=42)
    
    st.subheader("🔭 Generated Transit Light Curves")
    st.markdown("Each plot shows the simulated brightness changes as the planet transits its star.")
    
    for idx, row in sample_df.iterrows():
        period = row[period_col]
        duration = row[duration_col]
        depth = row[depth_col]
        disposition = row[disposition_col] if disposition_col else "Unknown"
        
        if pd.isna(period) or pd.isna(duration) or pd.isna(depth):
            continue
        
        if period <= 0 or duration <= 0 or depth <= 0:
            continue
        
        time, flux = generate_transit_lightcurve(period, duration, depth, num_points=2000, num_transits=3)
        ...
        for i in range(3):
            transit_center = period * (i + 0.5)
            transit_times.append(transit_center)
            fig.add_vline(
                x=transit_center, 
                line_dash="dash", 
                line_color="red", 
        ...
        fig.update_layout(
            title=f"Object {idx} - {disposition}<br><sub>Period: {period:.2f}d | Duration: {duration:.2f}h | Depth: {depth:.0f} ppm</sub>",
        ...
        fig.update_yaxes(range=[min(flux)*0.9995, max(flux)*1.0005])
        
        st.plotly_chart(fig, use_container_width=True)
        
        with st.expander(f"📊 Details for Object {idx}"):
            col_a, col_b, col_c, col_d = st.columns(4)
            col_a.metric("Orbital Period", f"{period:.2f} days")
            col_b.metric("Transit Duration", f"{duration:.2f} hours")
            col_c.metric("Transit Depth", f"{depth:.0f} ppm")
            col_d.metric("Disposition", disposition)
            
            st.markdown(f"""
            **What you're seeing:**
            - Each dip represents the planet passing in front of the star
            - The depth of {depth:.0f} ppm means the planet blocks {depth/1e4:.4f}% of the star's light
            - Transits repeat every {period:.2f} days (the orbital period)
            - Each transit lasts {duration:.2f} hours
            """)
    
    st.divider()
    st.subheader("📈 Transit Depth Distribution")
    
    fig_dist = px.histogram(
        df_filtered,
        x=depth_col,
        color=disposition_col if disposition_col else None,
        nbins=50,
        title="Distribution of Transit Depths",
        labels={depth_col: "Transit Depth (ppm)"},
        template='plotly_white'
    )
    fig_dist.update_layout(height=400)
    st.plotly_chart(fig_dist, use_container_width=True)
    
    st.subheader("🌍 Period vs Transit Duration")
    
    fig_scatter = px.scatter(
        df_filtered.sample(n=min(500, len(df_filtered)), random_state=42),
        x=period_col,
        y=duration_col,
        color=disposition_col if disposition_col else None,
        size=depth_col,
        hover_data=[depth_col],
        title="Orbital Period vs Transit Duration",
        labels={
            period_col: "Orbital Period (days)",
            duration_col: "Transit Duration (hours)",
            depth_col: "Transit Depth (ppm)"
        },
        template='plotly_white',
        log_x=True
    )
